...
const ItemCard = ({ item }) => {// Recibo el item como prop desde el componente padre. Que representa un juego en la lista de juegos.
  const navigate = useNavigate();// Hook de React Router para navegar entre rutas.

  return (// Componente que representa una tarjeta de un juego.
    <div className="bg-blue-200 p-4 rounded-lg shadow-md text-center overflow-hidden transform transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:bg-blue-300  flex flex-col h-full">
      ...
      <button onClick={() => navigate(`/items/${item.id}`)} // Navego a la ruta del detalle del juego al hacer clic en el botón.
